...
`data/products_500plus.csv` is a hand-curated catalog — product names, categories, subcategories, tags, and brands were chosen manually to resemble a real Indian grocery listing; only the `price` and `rating` columns are synthetically assigned per category. `data/user_ratings.csv` is fully synthetic end to end: all 150 users and all 6,796 ratings are generated by a seeded statistical model, and no real user or purchase history is behind any of them. Both generation processes are documented and reproducible in code — see [`data/README.md`](data/README.md), `data/generate_catalog.py`, and `data/generate_ratings.py`.
